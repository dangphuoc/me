...
1. Client grabs a connection from the pool
2. Sends request to server
3. Server processes, returns response
4. Server calls `response.close()` → **silently kills the TCP connection**
5. Client doesn't notice, happily puts the "dead" connection back in pool

**Request 2** - Disaster strikes:
1. Client grabs that same connection (thinking it's alive)
2. Tries to send data through a dead line
3. **Boom** - "Connection was closed"

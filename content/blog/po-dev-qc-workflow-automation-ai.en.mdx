...
## 4 PM Friday, and my ticket still hasn't been reviewed

Minh — a developer on my team — just pinged me on Google Chat.

*"Hey, the checkout flow ticket I finished Wednesday morning. I commented on Jira, tagged Huong, sent a demo video. But still no PO review. I don't know if she's seen it or just busy. And QC is waiting too, not sure if they should test first..."*

I look at the clock. Friday, almost end of day.

A simple ticket, coded in half a day, stuck for 2 days because... nobody knows who's waiting for whom.

Sound familiar? If you've worked in Agile teams long enough, you've definitely lived through this — either as Minh, or as Huong the PO drowning in 47 notifications not knowing which one to handle first.

![Process](https://images.unsplash.com/photo-1552664730-d307ca884978?w=800)

## The spiral everyone sees but no one stops

Let's zoom in on what's actually happening.

**Wednesday morning, 10:30.**

Minh finishes coding. Opens Jira, moves ticket to "Ready for QC". Then thinks: *"Huong often misses Jira notifications, better send a message too."*

Opens Google Chat, types: *"Hey Huong, ticket ABC-123 is done. Here's the demo video..."* Attaches 3-minute video. Send.

**11:15.**

Huong is in a meeting with stakeholders. Sees notification blinking but can't check immediately. *"I'll look later."*

**14:00.**

Meeting ends. Huong opens Google Chat, sees 23 unread messages. Scrolls through, sees Minh's message but there are more urgent things. *"I'll get to it."*

**17:00.**

Day ends. Huong forgets Minh's message because it got pushed down by 15 other messages.

**Thursday, 9:30.**

Minh checks Jira, ticket still "Ready for QC". QC is waiting for PO review first. *"Should I remind her?"*

Sends another message: *"Hey Huong, can you check the ticket I sent yesterday?"*

**10:00.**

Huong sees it, opens the demo video. Has some feedback. Types right into Google Chat: *"Minh, this button should be blue, and this flow doesn't handle the case when user isn't logged in."*

**10:30.**

Minh reads feedback. *"Right, but I need to copy this to Jira for tracking, otherwise I'll forget."* Opens Jira, pastes feedback into comment, tags Huong to confirm.

**11:00.**

Lan — the team's QC — sees ticket still "Ready for QC" with no updates. *"Has PO reviewed? Should I just test first?"*

She decides to wait, worried she'll have to retest if PO requests changes.
...
Do you see it? **Nobody did anything wrong.** Everyone's trying to follow the process. But the process itself is creating friction.
...
I asked: *"If AI writes the message, will Huong read it faster?"*

Silence.

*"Is the problem that messages aren't well-written, or that Huong doesn't know which one to read first?"*
...
Back to Minh and Huong's story.

The real problem isn't lack of AI to write messages. The problem is **nobody knows where information lives**.

Minh comments on Jira, then sends Google Chat, then attaches a separate video. Huong doesn't know where to look first. Lan the QC doesn't know if PO has reviewed because feedback is on Chat not Jira.
...
Huong opens Jira, everything's there. No need to read Chat. No need to ask back. No need for separate video through message.

Lan QC also knows exactly where to find information, and can immediately see if PO has commented.
...
OK, team now has convention. But Huong can still forget to check Jira because she's busy.

This is where automation comes in — but simple automation, not AI.

**Webhook + Bot:**

When Minh moves ticket to "Ready for QC", Jira automatically sends message to team's Google Chat space:

> *"🔔 Ticket ABC-123 is Ready for QC. Assignee: @Lan. [Jira Link]*"

Minh doesn't need to type anything. System does it.

**Timer + Reminder:**

If ticket is in "Ready for PO Review" for over 24 hours with no action from Huong, bot auto-sends reminder:

> *"⏰ Ticket ABC-123 waiting for PO review over 24h. @Huong*"

Over 48 hours? Escalate to shared channel and tag Scrum Master too.

**Daily Summary:**

Every morning at 8:30, bot posts to Chat:
...
Team reads 5 minutes before standup, standup only discusses blockers instead of everyone reading status again.
...
Huong presents a new user story: *"User can filter products by price, brand, rating, and color. Can select multiple filters at once."*

Sounds simple. But Lan QC starts getting a headache.
...
This is where AI is truly useful. Lan pastes the user story, AI generates draft 47 test cases — happy path, edge case, combination case, performance case. Lan reviews in 2 hours, adjusts a few things, done.
...
This is the bug report Minh received from... Lan herself, on a day when deadline was crushing.

*"Lan, what does 'doesn't work' mean? Which step? What environment? What error?"*

*"Sorry Minh, I'm testing 5 tickets at once, no time to write details."*

Neither is wrong. Lan is under pressure. Minh needs information.

AI can help here. Lan just describes briefly or sends screenshot, AI generates fully formatted bug report:
...
Lan reviews 30 seconds, submits. Minh reads and knows exactly what to do.
...
Huong writes requirement for new feature: *"User can delete paid orders within 24 hours."*
...
AI could catch this **before Dev starts coding**. It reads the new requirement, compares with all existing features and policies in the system, then flags:

...
Every 30 seconds, a job quietly pings the **health check** of each service. Response 200 means OK, anything else increments the consecutive failure count. Hit 3 in a row, alert fires to Slack and PagerDuty. Pure number comparison. No context needed.
...
Alongside that, another job runs every minute to **monitor resource usage**. It queries Prometheus: CPU > 85% for 5 continuous minutes means warning, > 95% for 3 minutes means critical. RAM and disk have their own thresholds. Write once, run forever.

At the start of each day, a job scans the **certificate and domain expiry** list. 30 days left means email warning. 7 days means Slack critical. Simple date arithmetic, nothing ambiguous.

The messaging layer has its own guardian: every minute, a job checks **queue depth** in RabbitMQ or SQS. If message count > 10,000 and consumer count < 3, alert *"queue is backlogging."* Fixed logic. No need to understand what's in the messages.

For databases, every 30 seconds a job queries **replication lag** between primary and replica. Lag > 5 seconds means warning. > 30 seconds means critical. Replica stops replicating means emergency. Number versus number, completely deterministic.

There's also the **synthetic transaction** job running every 5 minutes: create test order → verify it appears in database → verify email notification sent → cleanup. Pass or fail. No gray area. Script doesn't change between runs.

For a higher-level view, every hour a job counts **ERROR log volume** per service, compares with the 7-day average for the same time slot. Exceeds 3x, alert. Just basic statistics, no need to read log content.

And at the end of each week, a job aggregates uptime data, calculates **SLA percentage** per service, renders to a report and sends to stakeholders. Input is numbers, output is tables, completely fixed flow.
...
Back to the opening story: 47 alerts at once. Cron job can only send 47 separate notifications. On-call engineer wakes up to 47 messages with no idea where to start. Agent is different. It reads all 47 alerts, looks at the timeline, **auto-correlates to find root cause**: DB connection pool exhausted at 3:01, all dependent services timeout, queues backlog, 5xx rises. Agent consolidates into **one message**: *"Root cause is likely DB connection pool exhausted on db-primary-01. 46 other alerts are domino effects."* Each incident has a different alert combination. No rule can cover them all.

System generates thousands of error log lines daily. Cron job counts them, but Agent **reads and understands log content**. `NullPointerException at PaymentService.processRefund` appearing 200 times in 10 minutes — **serious**, directly affects users. `TimeoutException at RecommendationService` appearing 500 times — **low severity**, only affects recommendations. Agent classifies based on **understanding business impact**, not just counting.

After incidents, there's postmortem writing. Information is scattered: PagerDuty history, CI/CD logs, Slack conversation, git commits, metric graphs. Agent collects everything and **auto-writes incident summary and timeline**, accurate to the minute — who did what, what the impact was. Each incident unfolds differently, can't be templated.

2 AM, on-call engineer gets an alert, mind still foggy. Agent analyzes symptoms and **suggests runbook actions**: *"These symptoms resemble INC-4521 last month. Root cause then was memory leak after deploying version 3.2.1. Step 1: check memory trend of pod X. Step 2: if confirmed, rollback deployment Y."* Agent **understands pattern similarity** between incidents, not just keyword matching.

API gateway p99 latency increased from 200ms to 800ms. But here's where it gets complex: only requests from **Asia region**, only during **18h–22h**, only for users with **cart size > 10 items**. Cron job only sees "latency up." Agent does **multi-dimensional analysis**, drills down, hypothesizes: *"Possibly CDN node in Singapore, or query calculating prices for large carts is doing full table scan after recent migration."*

When incidents happen, beyond technical fix there's communication. Status page needs updating. Customer support needs briefing. PM needs to know which features are affected. Agent **drafts appropriate messages for each audience** — technical details for engineering, business impact for PM, user-facing message for status page.

Alert fires: *"Error rate increased 300%."* Reality: went from 0.001% to 0.003% — still very low, just small numbers making percentage look scary. Or spike from batch job retrying, not real users. Agent **assesses whether alert is false positive**: *"Absolute rate still acceptable. Spike from batch retry. No escalation needed."* Cron job can't make judgments like this.

And there's **smart scaling decisions** — not simple CPU-based auto-scaling. More like: *"Tonight there's a flash sale, traffic pattern will differ, pre-scale 2 hours ahead."* Or: *"Traffic is increasing but it's bot crawling, don't scale, just rate limit."* Agent analyzes traffic source, business calendar, historical patterns holistically then decides.

...
Sau khi áp dụng công thức right-sizing cho toàn bộ services:
...
Với một hệ thống phức tạp gồm hàng chục modules, tiến độ migration theo chiến lược soft migration:

| Stack | Tỷ lệ | Ghi chú |
|-------|-------|---------|
| **V3 Native (GraalVM)** | ~10% | Các modules đơn giản, stateless |
| **V3 JVM (Quarkus)** | ~15% | Modules phức tạp hơn |
| **V2 (Vert.x + Dagger)** | ~75% | Đang dần chuyển đổi |

Kết quả đạt được:
- **100% services mới** được viết trên V3 framework
- **Documentation đầy đủ** cho việc onboarding
- **Phased rollout** giảm thiểu risk

### Native Code: Từ 250MB xuống 25MB

Và đây là phần thú vị nhất.

Team đã thử nghiệm build native code cho một internal service - một trong những modules đầu tiên chạy Native trên production.
...
**Tự hào thứ hai:** Native code **thực sự works**. 250MB xuống 25MB không phải marketing material - đó là thực tế đo đạc được trên production.

## Tiếp theo là gì?

Hành trình chưa kết thúc. Với những kết quả đã đạt được, hướng đi tiếp theo khá rõ ràng:

**1. Mở rộng Native Code Coverage**

Bắt đầu từ những modules đơn giản, dần dần mở rộng sang các modules phức tạp hơn. Mỗi module native thành công là một bước tiến về hiệu suất và chi phí.

**2. Hoàn thiện V3 Migration**

Tiếp tục chuyển đổi các modules còn lại sang V3 framework, đảm bảo đầy đủ:
- Unit tests và integration tests
- Fault tolerance (Rate limit, Bulkhead, Circuit breaker)
- Observability (Health checks, Metrics, Tracing)

**3. Duy trì Culture Optimization**

Right-sizing không phải việc làm một lần. Cần review resource usage định kỳ, alert khi có services sử dụng resource bất thường (quá cao hoặc quá thấp).

---
title: "Don't Use AI For Tasks That Cron Jobs Do Better"
...
# Don't Use AI For Tasks That Cron Jobs Do Better

## It started in a design review

Last week, during a design review session, a developer presented a solution for what seemed like a simple task: **every morning, aggregate sprint metrics from Jira, CI/CD status from Jenkins, then post a summary to Google Chat for the team.**

They proposed using an AI Agent.

"The Agent will read data from various sources, analyze whether the sprint is healthy, then write a summary with intelligent insights."

Sounds great. Very "2026." But my "over-engineering" radar started beeping.

I asked: "Do the metrics ever change? Today velocity, tomorrow cycle time?"

"No, always: tickets done, tickets in progress, blocked tickets, build status."

"Does the aggregation method change?"

"No, always count and list them."
...
A cron job querying Jira API and Jenkins API every morning, formatting into a fixed message, posting to Chat — **there's no reason to call an LLM.** Higher cost. Higher latency. Non-deterministic output. And debugging? Good luck.
...
Example: daily ticket count varies. Today 5 tickets done, tomorrow 20 tickets. Build failed 3 times today, zero yesterday. Data "fluctuates." But the way you aggregate stays the same: count tickets by status, list blocked items, show build status.

Many people see "multiple sources" and think they need AI. **But multiple sources doesn't mean ambiguous.** Even pulling from 10 different APIs, if the processing is deterministic, traditional jobs are enough.
